"""
Functions for handling ENCODE metadata files
"""
...
def add_TF(encode_file, outfile, species):
    ...
    Parameters
    ----------------
    encode_file: str ot pd.DataFrame
        the metafile download from encode
        required colunmn name in input: "Experiment target"
    outfile: str or None
    species: str, eg. "dmelanogaster", "celegans", "human". The string that used as the prefix in the "Experiment target" column
    ...
    # read input
    df = __read_df_or_str(encode_file)
...
def filter_subset(encode_file, outfile, rm_poly=True, rs=200, **kwargs):
    """
    Filter metadata to keep certain files
    ...
    outfile: str or None
    data_type: only support human in format of "human-GM12878"/"human-K562"
    rm_poly: boolean
...
def download_file(encode_file, download_dir, ext=".bed.gz", existed_data_dir=None):
    ...
    for dl, file_accession in zip(dls, file_accessions):
        ...
        dst = "%s/%s%s" % (download_dir, file_accession, ext)

        if not os.path.isfile(dst):
            # if file not already existed
            if existed_data_dir:
                existed_dst = "%s/%s%s" % (existed_data_dir, file_accession, ext)
...
def unique_TF_parser(encode_file, motif=True, prior=None):
    ...
    prior: str
        if given, will further filter for 'Biosample term name'==prior
